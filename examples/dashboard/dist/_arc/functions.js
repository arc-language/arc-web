...
// @server fn incrementCounter
async function _handler_incrementCounter(req) {
  try {
    // Parse ADP or JSON request body
    const _body = req.method === 'POST' ? await _parseBody(req) : {}
    const by = Object.prototype.hasOwnProperty.call(_body,'by') ? _body['by'] : undefined
    const _session = req._arc_session ?? {}  // injected by auth middleware
    const _result = await (async function() {
      return (count+by);
    })()
    ...
    console.error('[arc] @server incrementCounter error:', _e)
    return new Response(JSON.stringify({ error: 'Internal server error' }), {
      status: 500, headers: { 'Content-Type': 'application/json' }
    })
  }
...
export default {
  ...
  if (_path === '/_arc/fn/incrementCounter') return _handler_incrementCounter(req)
    return new Response('Not Found', { status: 404 })
  }
}

// Node.js / Bun / Deno adapter
if (typeof module !== 'undefined') module.exports = { _handler_getStats, _handler_incrementCounter }