...
async function incrementCounter(by) {
  const _res = await fetch('/_arc/fn/incrementCounter', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-adp'},
    body: _adpEncode({by})
  })
...
document.getElementById('_a2').addEventListener('click',function(event){_set_count(_count-(1));});
document.getElementById('_a3').addEventListener('click',function(event){_set_count(_count+(1));});
_el__a1.textContent=_count;
})();
